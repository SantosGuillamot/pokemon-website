...
- **Moves/items not seeded**: large datasets, not needed for the initial Pokemon search page. Add later when the damage calculator is built
...
| 1.2 | Schema: `types` (single table with matchup jsonb arrays + color) | ⬜ Redo |
| 1.3 | Schema: `abilities` (id, name, effect) | ⬜ Redo |
| 1.4 | Schema: `natures` (id, name, increased_stat, decreased_stat) | ⬜ Redo |
| 1.5 | Schema: `moves` (power, accuracy, pp, priority, effect, effect_chance, type_id, damage_class, target, min_hits, max_hits, flags jsonb) | ⬜ Redo |
| 1.6 | Schema: `items` (name, image_url, category, effect, meta jsonb) — competitive items only | ⬜ Redo |
| 1.7 | Schema: `pokemon` (dex_number, name, images jsonb, 6 base stats, weight, height) | ⬜ Redo |
| 1.8 | Schema: `pokemon_types` (pokemon_id, type_id, slot) | ⬜ Redo |
| 1.9 | Schema: `pokemon_abilities` (pokemon_id, ability_id, is_hidden, slot) | ⬜ Redo |
| 1.10 | Schema: `pokemon_moves` (pokemon_id, move_id) — new junction table | ⬜ Todo |
| 1.11 | Remove `type_matchups`, `status_conditions`, `field_conditions` schema files | ⬜ Todo |
| 1.12 | Seed script via PokéAPI (types with matchups, abilities, natures, 151 Pokemon + types/abilities/moves) | ⬜ Redo |
